...
async function mvc(answer: Answers, pathName: string) {
  ...
  const files = [
    ...
    {
      name: 'controllers/user.controller',
      content: generateUsercontroller(module, language, architecture),
    },
    {
      name: 'services/user.services',
      content: generateUserservices(module, language, architecture),
    },
    ...
    {
      name: 'repositorys/user.repository',
      content: generateUserRepository(module, language, architecture),
    },
  ];
  ...
  if (logger !== 'none')
    files.push({ name: 'config/logger', content: generateLogger(module, language, logger) });
...
async function feature(answer: Answers, pathName: string) {
  ...
  const files = [
    ...
    { name: 'utils/asynchandler', content: asynchandler(module, language) },
    {
      name: 'modules/user/user.routes',
      content: generateUserroutes(module, language, architecture),
    },
    { name: 'modules/user/user.validation', content: '' },
    {
      name: 'modules/user/user.controller',
      content: generateUsercontroller(module, language, architecture),
    },
    {
      name: 'modules/user/user.services',
      content: generateUserservices(module, language, architecture),
    },
    {
      name: 'modules/user/user.repository',
      content: generateUserRepository(module, language, architecture),
    },
  ];
