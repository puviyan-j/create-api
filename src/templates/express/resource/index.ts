...
export async function generatemodule(answer: Answers, data: ResourceGenerator) {
  ...
  const handler = await fs.exists(`utils/asynchandler.${ext}`);

  if (!handler) {
    await fs.ensureFile(`src/utils/asynchandler.${ext}`);
    await fs.writeFile(
      `src/utils/asynchandler.${ext}`,
      asynchandler(answer.module, answer.language),
    );
  }
...
const mvc = (data: ResourceGenerator, ext: '.js' | '.ts', havevalidation: boolean) => {
  const files = [
    ...
    { path: `src/services/${data.name}.service${ext}`, content: generateservices },
    { path: `src/controllers/${data.name}.controller${ext}`, content: generatecontroller },
    { path: `src/routes/${data.name}.routes${ext}`, content: generateroutes },
  ];

  if (ext === '.ts')
    files.push({ path: `src/types/${data.name}.types${ext}`, content: generatetypes });
  if (havevalidation)
    files.push({ path: `src/validations/${data.name}.validation${ext}`, content: generatetypes });
...
const future = (data: ResourceGenerator, ext: '.js' | '.ts', havevalidation: boolean) => {
  ...
  if (ext === '.ts')
    files.push({
      path: `src/modules/${data.name}/${data.name}.types${ext}`,
      content: generatetypes,
    });
  if (havevalidation)
    files.push({
      path: `src/modules/${data.name}/${data.name}.validation${ext}`,
      content: generatetypes,
    });
